...
} from "@/shared/config/components-library";
...
export async function generateMetadata({
  params,
}: AnimationsPageProps): Promise<Metadata> {
  const { term } = await params;
  const { title, description } =
    animationsSEOTerms[term as AnimationsSEOTermsKeys];
  ...
  return {
    ...
    openGraph: {
      ...
      description:
        "A collection of ready-to-use animated UI components built with React, Tailwind CSS, Shadcn, and Framer Motion.",
...
export default async function Page({ params }: AnimationsPageProps) {
  const { term } = await params;
  const componentCategory = getComponentCategory(
    animationsSEOTerms[term as AnimationsSEOTermsKeys].slug,
  );
  if (!componentCategory) {
    return notFound();
  }
  const components = getComponentsByNames(
    componentCategory.components.map((item) => item.name),
  );
  return (
    <>
