...
function useSidebarContext() {
  ...
  if (!context) {
    throw new Error(
      "Sidebar components must be used within a Sidebar provider",
    );
  }
  return context;
}
...
export default function Navbar1() {
  ...
  return (
    <SidebarContext.Provider value={contextValue}>
      <TooltipProvider>
        <nav className="w-18 flex flex-col items-center bg-background border-r py-8 px-4">
...
const SidebarHeader = React.forwardRef<HTMLDivElement, SidebarHeaderProps>(
  ({ className, children, ...props }, ref) => (
    <div
      ...
      id="sidebar-header"
      className={cn(
        "flex flex-col gap-4 after:border-b after:border-b-border",
        className,
      )}
      {...props}
    >
...
const SidebarFooter = React.forwardRef<HTMLDivElement, SidebarFooterProps>(
  ({ className, children, ...props }, ref) => (
    <div
      ...
      id="sidebar-footer"
      className={cn(
        "flex flex-col gap-4 before:border-t before:border-t-border",
        className,
      )}
      {...props}
    >
...
const SidebarItem = React.forwardRef<HTMLButtonElement, SidebarItemProps>(
  (
    { itemKey, showActiveState = true, tooltip, className, children, ...props },
    ref,
  ) => {
    ...
    const content = (
      <button
        ...
        className={cn(
          ...
          isSelected
            ? "text-white"
            : "text-[#707070] hover:text-black hover:dark:text-white",
        )}
