...
export function Footer() {
  return (
    <footer className="relative border-t border-border/60 bg-background">
      ...
      <div
        aria-hidden="true"
        className="pointer-events-none absolute left-1/2 top-0 h-48 w-[36rem] -translate-x-1/2 rounded-full bg-foreground/[0.04] blur-3xl dark:bg-foreground/[0.07]"
      />
          ...
          <div className="grid gap-10 lg:grid-cols-[1.25fr_0.75fr_0.9fr]">
            <div className="space-y-6">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <Button
                  asChild
                  className="size-10 rounded-full border-border/70 bg-background/70 shadow-sm transition-transform hover:-translate-y-0.5 hover:bg-background"
                  variant="outline"
                  aria-label="GitHub"
                  size="icon"
                >
            ...
            <div className="space-y-4 font-sans">
              <h4 className="text-xs uppercase tracking-[0.35em] text-muted-foreground">
                Components
              </h4>
              <nav aria-label="Component links" className="grid gap-2">
                {componentLinks.map((link) => (
                  ...
                  >
                    ...
                    <span className="text-xs opacity-0 transition-transform duration-200 group-hover:translate-x-0.5 group-hover:opacity-100">
                      ↗
                    </span>
                  </Link>
                ))}
